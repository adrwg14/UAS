		...
		<div class="row">
			<div class="span12">
				...
				<div class="well well-small">
					<h1>Check Out <small class="pull-right">@<b><?= $_SESSION['email']; ?></b></small></h1>
					<hr class="soften" />
					...
					$sql = "SELECT * FROM keranjang WHERE email = '$email' AND status IS NULL";
					...
					// Periksa apakah tombol checkout ditekan
					if (isset($_POST['checkout'])) {
						$email = $_SESSION['email']; // Ambil email pengguna yang sedang login

						// Query untuk mengubah status menjadi "pembayaran selesai" berdasarkan email
						$sql = "UPDATE keranjang SET status = 'pembayaran selesai' WHERE email = '$email'";

						if ($conn->query($sql) === TRUE) {
							echo '<div class="alert alert-success">Berhasil Checkout! <a href="pesanan.php">Lihat Pesanan Disini</a></div>';
						} else {
					...
					<form action="" method="post">
						<button type="submit" name="checkout" class="btn w-100">Checkout</button>
					</form>
